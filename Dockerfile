...
# allow non-free, to install rar and unrar
...
# install 7zip and upx
COPY ./install-custom-packages.sh .
RUN ./install-custom-packages.sh
...
RUN uv pip uninstall --system azul-plugin-unbox
RUN uv pip install --system --no-deps --find-links /tmp/ azul-plugin-unbox==$(hatchling version)
...
RUN if [ "$GIT_BRANCH_NAME" = "refs/heads/dev" ] ; then \
    uv pip freeze | grep 'azul-.*==' | grep -v '^azul-plugin-unbox' | cut -d "=" -f 1 | xargs -I {} uv pip install --extra-index-url=$(UV_INDEX_URL) --system --upgrade --no-deps --prerelease allow '{}>=0.0.0-dev' \
    else \
    uv pip freeze | grep 'azul-.*==' | grep -v '^azul-plugin-unbox' | cut -d "=" -f 1 | xargs -I {} uv pip install --extra-index-url=$(UV_INDEX_URL) --system --upgrade --no-deps '{}>=0.0.0'\
    ;fi
